...
The work performed by the METAD_JARZ bias can be calculated using CALC_WORK, note that this is expensive when not using grids.
...
as done e.g. in Ref. \cite gil2015enhanced . This indeed can be obtained by using the METAD_JARZ
...
METAD_JARZ ARG=d1,d2 SIGMA=0.2,0.2 HEIGHT=0.3 PACE=500 LABEL=restraint
...
DISTANCE ATOMS=3,5 LABEL=d1
DISTANCE ATOMS=2,4 LABEL=d2
METAD_JARZ ARG=d1,d2 SIGMA=20 HEIGHT=0.3 PACE=500 LABEL=restraint ADAPTIVE=DIFF
PRINT ARG=d1,d2,restraint.bias STRIDE=100  FILE=COLVAR
\endplumedfile
...
DISTANCE ATOMS=3,5 LABEL=d1
DISTANCE ATOMS=2,4 LABEL=d2
METAD_JARZ ARG=d1,d2 SIGMA=0.05 HEIGHT=0.3 PACE=500 LABEL=restraint ADAPTIVE=GEOM
PRINT ARG=d1,d2,restraint.bias STRIDE=100  FILE=COLVAR
\endplumedfile
...
METAD_JARZ ARG=d SIGMA=0.1 TAU=4.0 TEMP=300 PACE=100 BIASFACTOR=1.0
\endplumedfile
...
\plumedfile
d: DISTANCE ATOMS=3,5
METAD_JARZ ARG=d SIGMA=0.1 TAU=4.0 TEMP=300 PACE=100 RECT=1.0,1.5,2.0,3.0
\endplumedfile
...
public:
  explicit MetaD_jarz(const ActionOptions&);
...
PLUMED_REGISTER_ACTION(MetaD_jarz,"METAD_JARZ")

void MetaD_jarz::registerKeywords(Keywords& keys) {
...
const std::string MetaD_jarz::tempering_names_[1][2] = {{"TT", "transition tempered"}};

void MetaD_jarz::registerTemperingKeywords(const std::string &name_stem, const std::string &name, Keywords &keys) {
...
MetaD_jarz::MetaD_jarz(const ActionOptions& ao):
  PLUMED_BIAS_INIT(ao),
  kbt_(0.0),
  stride_(0),
  calc_work_(false),
  ...
  nlist_steps_(0)
{
  ...
  if(fa_update_frequency_!=0 && !freq_adaptive_) {
    plumed_merror("It doesn't make sense to use the FA_MAX_PACE keyword if frequency adaptive METAD_JARZ hasn't been activated by using the FREQUENCY_ADAPTIVE flag");
  }
  if(fa_update_frequency_==0 && freq_adaptive_) {
  ...
  if(fa_min_acceleration_!=1.0 && !freq_adaptive_) {
    plumed_merror("It doesn't make sense to use the FA_MIN_ACCELERATION keyword if frequency adaptive METAD_JARZ hasn't been activated by using the FREQUENCY_ADAPTIVE flag");
  }
...
//    if (!grid_) error("Calculating the maximum bias on the fly works only with a grid");
  ...
  if(grid_) {
    if(!(gridreadfilename_.length()>0)) {
      ...
      for(unsigned i=0; i<getNumberOfArguments(); i++) {
        ...
        if(adaptive_==FlexibleBin::none) {
          if(mesh>0.5*sigma0_[i]) log<<"  WARNING: Using a METAD_JARZ with a Grid Spacing larger than half of the Gaussians width (SIGMA) can produce artifacts\n";
        } else {
          if(sigma0min_[i]<0.) error("When using ADAPTIVE Gaussians on a grid SIGMA_MIN must be specified");
          if(mesh>0.5*sigma0min_[i]) log<<"  WARNING: to use a METAD_JARZ with a GRID and ADAPTIVE you need to set a Grid Spacing larger than half of the Gaussians (SIGMA_MIN) \n";
        }
      }
      ...
      for(unsigned i=0; i<getNumberOfArguments(); ++i) {
        ...
        Tools::convert(gmax[i],b);
        double mesh=(b-a)/((double)gbin[i]);
        if(mesh>0.5*sigma0_[i]) log<<"  WARNING: Using a METAD_JARZ with a Grid Spacing larger than half of the Gaussians width can produce artifacts\n";
      }
  ...
  // it would introduce troubles when using replicas without METAD_JARZ
  ...
  if(getRestart()) {
    ...
    if(calc_max_bias_) {
      if(grid_) max_bias_ = BiasGrid_->getMaxValue();
      else log.printf(" +++ WARNING +++ without grid maxbias restarting will be inaccurate\n");
  ...
  for(const auto & p : actionSet) if(dynamic_cast<MetaD_jarz*>(p.get())) { concurrent=true; break; }
...
void MetaD_jarz::readTemperingSpecs(TemperingSpecs &t_specs)
{
...
void MetaD_jarz::logTemperingSpecs(const TemperingSpecs &t_specs)
{
...
void MetaD_jarz::readGaussians(IFile *ifile)
{
...
void MetaD_jarz::writeGaussian(const Gaussian& hill, OFile&file)
{
  unsigned ncv=getNumberOfArguments();
...
void MetaD_jarz::addGaussian(const Gaussian& hill)
{
  if(grid_) {
...
std::vector<unsigned> MetaD_jarz::getGaussianSupport(const Gaussian& hill)
{
...
double MetaD_jarz::getBias(const std::vector<double>& cv)
{
  double bias=0.0;
...
double MetaD_jarz::getBiasAndDerivatives(const std::vector<double>& cv, std::vector<double>& der)
{
  unsigned ncv=getNumberOfArguments();
  double bias=0.0;
  if(grid_) {
...
double MetaD_jarz::getGaussianNormalization(const Gaussian& hill)
{
  double norm=1;
...
double MetaD_jarz::evaluateGaussian(const std::vector<double>& cv, const Gaussian& hill)
{
...
double MetaD_jarz::evaluateGaussianAndDerivatives(const std::vector<double>& cv, const Gaussian& hill, std::vector<double>& der, std::vector<double>& dp_)
{
  unsigned ncv=cv.size();
...
double MetaD_jarz::getHeight(const std::vector<double>& cv)
{
...
void MetaD_jarz::temperHeight(double& height, const TemperingSpecs& t_specs, const double tempering_bias)
{
...
void MetaD_jarz::calculate()
{
  ...
  if(calc_work_) getPntrToComponent("instwork")->set(0);
  if(calc_max_bias_ && ene>max_bias_)
  {
    max_bias_=ene;
    getPntrToComponent("maxbias")->set(max_bias_);
  }
...
bool MetaD_jarz::scanOneHill(IFile* ifile, std::vector<Value>& tmpvalues, std::vector<double>& center, std::vector<double>& sigma, double& height, bool& multivariate)
{
  double dummy;
  multivariate=false;
...
void MetaD_jarz::computeReweightingFactor()
{
...
double MetaD_jarz::getTransitionBarrierBias()
{
...
void MetaD_jarz::updateFrequencyAdaptiveStride()
{
...
bool MetaD_jarz::checkNeedsGradients()const
{
