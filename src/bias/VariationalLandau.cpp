...
//+PLUMEDOC BIAS VARIATIONAL_LANDAU
/*
...
//now the bias class
class VariationalLandau : public Bias {
...
public:
  VariationalLandau(const ActionOptions&);
  void calculate();
  static void registerKeywords(Keywords& keys);
};

const unsigned int VariationalLandau::fwv_sizes_[max_n2_max_]= {3,9,13,16,28,40,40,46,61,73,85,89,101,125,125,128,152,170,182,194}; //not a problem if two have same size

PLUMED_REGISTER_ACTION(VariationalLandau,"VARIATIONAL_LANDAU")

void VariationalLandau::registerKeywords(Keywords& keys) {
...
VariationalLandau::VariationalLandau(const ActionOptions&ao):
  PLUMED_BIAS_INIT(ao)
{
...
//  amplitude_cutoff_=false; // for some testing only
...
void VariationalLandau::calculate()
{
...
bool VariationalLandau::check_arg_and_set_n2_max(unsigned int arg_number) //FIXME maybe here could put the automatic calculation of fwv_sizes_, so that any n2_max can be given
{
...
void VariationalLandau::initialize_conv_pair(unsigned int n2_max)
{
...
void VariationalLandau::update_target_av()
{
  for (unsigned int i=0; i<dim_; i++)
    target_av_LI_[i]=0;
...
void VariationalLandau::calc_landau_int_and_set_forces(double* lan_int)
{
//initialize to zero needed variables (important if parallel)
  double Kin_int=0.;
  double Quad_int=0.;
  double Quart_int=0.;
  std::vector<double> dQuart_dRe(fwv_size_);
  std::vector<double> dQuart_dIm(fwv_size_);
  ...
  for (unsigned int q=rank; q<conv_pair_.size(); q+=stride) //PARALLEL LOOP
  {
    ...
    if (q<fwv_size_)
    {
      const unsigned int k=q; //just to avoid confusion: is below fwv_size_
    ...
    //now with psi2_q can start building also the derivatives
    if (!no_quartic_)
    {
      if (q<fwv_size_)
      {
  ...
  Quart_int+=pow(Quad_int,2);//the q=(0,0,0) component is equal to Quad_int
...
//      const double force_Rk=-Vol_*(mean_coeff_[0]*dKin_dRe_k+mean_coeff_[1]*dQuad_dRe_k+mean_coeff_[2]*dQuart_dRe[k]);
//      const double force_Ik=-Vol_*(mean_coeff_[0]*dKin_dIm_k+mean_coeff_[1]*dQuad_dIm_k+mean_coeff_[2]*dQuart_dIm[k]);
      const double coef=-4*Vol_*(mean_coeff_[0]*k2_[k]+mean_coeff_[1]);
      const double force_Rk=coef*Re_[k]-mean_coeff_[2]*Vol_*dQuart_dRe[k];
      const double force_Ik=coef*Im_[k]-mean_coeff_[2]*Vol_*dQuart_dIm[k];
...
void VariationalLandau::update_ensemble_av(double *lan_int)
{
...
void VariationalLandau::update_omega_and_coeffs()
{
...
inline unsigned int VariationalLandau::get_index(unsigned int i, unsigned int j) const //mapping of a (dim_)x(dim_) symmetric matrix into a vector
{
  if (i<=j)
