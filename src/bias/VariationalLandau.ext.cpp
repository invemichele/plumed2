...
//+PLUMEDOC BIAS VARIATIONAL_LANDAU_EXT //XXX extended version, taking into account psi_zero_ as free parameter
/*
 ...
 *
 *  F = fes_coeff[0]*Ig + fes_coeff[1]*I2 + fes_coeff[2]*I3 + fes_coeff[3]*I4
 *
 *  g = fes_coeff[0]
 *  a = fes_coeff[1]-3/8*fes_coeff[2]*fes_coeff[2]/fes_coeff[3]
 *  b = fes_coeff[3]
 *  psi_zero = fes_coeff[2]/(4*fes_coeff[3])
 *
*/
//+ENDPLUMEDOC
...
//now the bias class
class VariationalLandau_EXT : public Bias {
...
private:
  ...
  static const unsigned int dim_=4; //number of Landau Integrals (LI). is a static const mainly as a reminder that you cannot change it unless you change other parts of the code
...
public:
  VariationalLandau_EXT(const ActionOptions&);
  void calculate();
  static void registerKeywords(Keywords& keys);
};

const unsigned int VariationalLandau_EXT::fwv_sizes_[max_n2_max_]= {3,9,13,16,28,40,40,46,61,73,85,89,101,125,125,128,152,170,182,194}; //not a problem if two have same size

PLUMED_REGISTER_ACTION(VariationalLandau_EXT,"VARIATIONAL_LANDAU_EXT")

void VariationalLandau_EXT::registerKeywords(Keywords& keys) {
  ...
  keys.addOutputComponent("I3","default","the Landau integral given by the cubic terms");
...
VariationalLandau_EXT::VariationalLandau_EXT(const ActionOptions&ao):
  PLUMED_BIAS_INIT(ao)
{
...
//add all the output components
  ...
  addComponent("I3"); componentIsNotPeriodic("I3"); valueI3=getPntrToComponent("I3");
...
void VariationalLandau_EXT::calculate()
{
...
//else go haed and calculate landau integrals and forces
  ...
  valueI3->set(Landau_int[2]);
  valueI4->set(Landau_int[3]);
...
bool VariationalLandau_EXT::check_arg_and_set_n2_max(unsigned int arg_number) //FIXME maybe here could put the automatic calculation of fwv_sizes_, so that any n2_max can be given
{
...
void VariationalLandau_EXT::initialize_conv_pair(unsigned int n2_max)
{
...
void VariationalLandau_EXT::update_target_av()
{
  for (unsigned int i=0; i<dim_; i++)
    target_av_LI_[i]=0;
...
//the average is done in a uniform way or semi-WT dropping the I3 and I4 terms
  ...
  { //semi-well-tempered with Fes'=g*Ig+a*I2 instead of Fes=g*Ig+a*I2+b*I4+c*I3
    for (unsigned int k=0; k<fwv_size_; k++)
    {
...
//now use them to evaluate the averages of the integrals
  for (unsigned int q=0; q<conv_pair_.size(); q++)
  {
    if (q<fwv_size_)
    {
      ...
      target_av_LI_[3]+=4*av_r4[k];
      for (unsigned int kk=k+1; kk<fwv_size_; kk++)
        target_av_LI_[3]+=8*av_r2[k]*av_r2[kk];
    }
    for (unsigned int h=0; h<conv_pair_[q].size(); h++)
    {
      ...
      if(k==kk)
        target_av_LI_[3]+=2*av_r4[k];
      else
        target_av_LI_[3]+=8*av_r2[k]*av_r2[kk];
    }
  }
  target_av_LI_[1]+=pow(psi_zero_,2);
  target_av_LI_[2]+=psi_zero_*target_av_LI_[1];
  target_av_LI_[3]+=pow(psi_zero_,2)*(6*target_av_LI_[1]-5*pow(psi_zero_,2));

  log.printf(" Averages over the uniform target distribution: <Ig>=%f\t<I2>=%f\t<I3>=%f\t<I4>=%f\n",target_av_LI_[0],target_av_LI_[1],target_av_LI_[2],target_av_LI_[3]);
  //NB I3 has always zero average
}

void VariationalLandau_EXT::calc_landau_int_and_set_forces(double* lan_int)
{
//initialize to zero needed variables (important if parallel)
  ...
  double Cubic_int=0.;
  double Quart_int=0.;
  std::vector<double> dCubic_dRe(fwv_size_,0.);
  std::vector<double> dCubic_dIm(fwv_size_,0.);
  std::vector<double> dQuart_dRe(fwv_size_,0.);
  std::vector<double> dQuart_dIm(fwv_size_,0.);
  ...
  {
    //first build psi2_q (actually these are rescaled: V*psi2_q, V*d_psi2_q)
    double Re_psi2_q=0.;
    double Im_psi2_q=0.;
    for (unsigned int h=0; h<conv_pair_[q].size(); h++)
    {
      ...
      const unsigned int k=conv_pair_[q][h].k;
      const unsigned int kk=conv_pair_[q][h].kk;
      const int sign_k=conv_pair_[q][h].sign_k;
      const int sign_kk=conv_pair_[q][h].sign_kk;
    ...
    if (q<fwv_size_)
    {
      ...
      //now with psi2_q we can build the integrals...
      Kin_int+=k2_[k]*(pow(Re_[k],2)+pow(Im_[k],2));
      Quad_int+=pow(Re_[k],2)+pow(Im_[k],2);
      Cubic_int+=Re_[k]*Re_psi2_q+Im_[k]*Im_psi2_q;
      //... and their derivatives
      dCubic_dRe[k]+=6*Re_psi2_q;//not easy to see why, but it is right
      dCubic_dIm[k]+=6*Im_psi2_q;
      dQuart_dRe[k]+=psi_zero_*Re_psi2_q;
      dQuart_dIm[k]+=psi_zero_*Im_psi2_q;
    }
    Quart_int+=pow(Re_psi2_q,2)+pow(Im_psi2_q,2);
    for (unsigned int h=0; h<conv_pair_[q].size(); h++)
    {
      const unsigned int k=conv_pair_[q][h].k;
      const unsigned int kk=conv_pair_[q][h].kk;
  ...
  Cubic_int*=2;
  Quart_int*=2;
  Quad_int+=pow(psi_zero_,2);
  Cubic_int+=psi_zero_*Quart_int;
  Quart_int+=pow(Quad_int,2);//the q=(0,0,0) component is equal to Quad_int^2
...
//set the forces
  if (!no_bias_)
  {
    ...
    for (unsigned int k=0; k<fwv_size_; k++)
    {
      //finish building the derivatives...
      const double  dKin_dRe_k=4*Re_[k]*k2_[k];
      const double  dKin_dIm_k=4*Im_[k]*k2_[k];
      const double dQuad_dRe_k=4*Re_[k];
      const double dQuad_dIm_k=4*Im_[k];
      dCubic_dRe[k]+=psi_zero_*dQuad_dRe_k;
      dCubic_dIm[k]+=psi_zero_*dQuad_dIm_k;
      dQuart_dRe[k]+=Quad_int*Re_[k];
      dQuart_dIm[k]+=Quad_int*Im_[k];
      dQuart_dRe[k]*=8;
      dQuart_dIm[k]*=8;
      ...
      double force_Rk=-Vol_*(mean_coeff_[0]*dKin_dRe_k+mean_coeff_[1]*dQuad_dRe_k+mean_coeff_[2]*dCubic_dRe[k]+mean_coeff_[3]*dQuart_dRe[k]);
      double force_Ik=-Vol_*(mean_coeff_[0]*dKin_dIm_k+mean_coeff_[1]*dQuad_dIm_k+mean_coeff_[2]*dCubic_dIm[k]+mean_coeff_[3]*dQuart_dIm[k]);
...
void VariationalLandau_EXT::update_ensemble_av(double *lan_int)
{
...
void VariationalLandau_EXT::update_omega_and_coeffs()
{
...
inline unsigned int VariationalLandau_EXT::get_index(unsigned int i, unsigned int j) const //mapping of a (dim_)x(dim_) symmetric matrix into a vector
{
  if (i<=j)
