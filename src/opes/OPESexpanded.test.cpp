...
Such ensembles can be sampled via methods like replica exchange, or this \ref OPES_EXPANDED_TEST bias action.
...
Contrary to \ref OPES_METAD, \ref OPES_EXPANDED_TEST does not use kernel density estimation.
...
The \ref OPES_EXPANDED_TEST bias will create a multidimensional target grid to sample all the combinations.
...
opes: OPES_EXPANDED_TEST ARG=(ecv_.*) PACE=500 WALKERS_MPI PRINT_STRIDE=1000
...
//miscellaneous
  keys.addFlag("SUBTRACT_CVS",false,"subtract the value of the CVs to the bias. To be used when CVs are biases");
...
//parallelization stuff
  ...
  parseFlag("SUBTRACT_CVS",subtract_cvs_);
...
//printing some info
  ...
  if(subtract_cvs_)
    log.printf(" -- SUBTRACT_CVS: remove the value of the CVs from the final bias\n");
...
//set bias and forces
  ...
  if(subtract_cvs_)
  {
    double removed_bias=0;
    for(unsigned j=0; j<ncv_; j++)
      removed_bias+=getArgument(j);
    setBias(bias-removed_bias);
    for(unsigned j=0; j<ncv_; j++)
      setOutputForce(j,-1+kbt_*der_sum_cv[j]/sum);
  }
  else
  {
    setBias(bias);
...
void OPESexpanded_test::update()
{
  ...
  if(getStep()%stride_==0)
  {
    ...
    //update averages
    double current_bias=getOutputQuantity(0); //the first value is always the bias
    if(subtract_cvs_)
    {
      for(unsigned j=0; j<ncv_; j++)
        current_bias+=getArgument(j); //add back the other biases
    }
...
void OPESexpanded_test::init_fromObs() //This could probably be faster and/or require less memory...
{
...
void OPESexpanded_test::printDeltaF()
{
...
void OPESexpanded_test::dumpStateToFile()
{
...
void OPESexpanded_test::updateDeltaF(double bias)
{
...
double OPESexpanded_test::getExpansion(unsigned i) const
{
  double expansion=0;
  for(unsigned j=0; j<ncv_; j++)
