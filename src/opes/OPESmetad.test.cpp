...
It is possible to take into account also of other bias potentials besides the one of \ref OPES_METAD_TEST during the internal reweighting for \f$P(\mathbf{s})\f$ estimation.
...
Another possible usage of EXTRA_BIAS is to make sure that \ref OPES_METAD_TEST does not push against another fixed bias added to restrain the CVs range (e.g. \ref UPPER_WALLS).
...
template <class mode>
class OPESmetad_test : public bias::Bias {
...
public:
  explicit OPESmetad_test(const ActionOptions&);
...
typedef OPESmetad_test<convergence> OPESmetad_test_c;
PLUMED_REGISTER_ACTION(OPESmetad_test_c,"OPES_METAD_TEST")
...
Intuitively, while \ref OPES_METAD_TEST aims at quickly converging the reweighted free energy, \ref OPES_METAD_TEST_EXPLORE aims at quickly sampling the target well-tempered distribution.
...
typedef OPESmetad_test<exploration> OPESmetad_test_e;
// For some reason, this is not seen correctly by cppcheck
// cppcheck-suppress unknownMacro
PLUMED_REGISTER_ACTION(OPESmetad_test_e,"OPES_METAD_TEST_EXPLORE")

template <class mode>
void OPESmetad_test<mode>::registerKeywords(Keywords& keys)
{
...
//miscellaneous
  keys.addFlag("GLOBAL_BIAS",false,"consider all the defined biases for reweighting");
...
template <class mode>
OPESmetad_test<mode>::OPESmetad_test(const ActionOptions& ao)
  : PLUMED_BIAS_INIT(ao)
  , isFirstStep_(true)
  , counter_(1)
...
//options involving extra arguments
  ...
  parseFlag("GLOBAL_BIAS",global_bias_);
...
//  if(global_bias_)
//  {
//    log.printf(" -- GLOBAL_BIAS: for reweighting the following biases are used:");
//    std::vector<ActionWithValue*> tmpActions=plumed.getActionSet().select<ActionWithValue*>();
//    for(unsigned a=0; a<tmpActions.size(); a++)
//      if(tmpActions[a]->exists(tmpActions[a]->getLabel()+".bias"))
//        log.printf("  %s",tmpActions[a]->getName().c_str());
//    log.printf("\n");
//  }
...
template <class mode>
void OPESmetad_test<mode>::calculate()
{
//get cv
...
template <class mode>
void OPESmetad_test<mode>::update()
{
...
//do update
  if(getStep()%stride_==0 && (excluded_region_==NULL || excluded_region_->get()==0))
  {
    ...
    if(global_bias_)
    {
      std::vector<ActionWithValue*> tmpActions=plumed.getActionSet().select<ActionWithValue*>();
      for(unsigned a=0; a<tmpActions.size(); a++)
//        if(tmpActions[a]->getName().find("global")!=std::string::npos)
        log_weight+=tmpActions[a]->getOutputQuantity("bias")/kbt_; //return zero if it does not exist
    }
    else
    {
      log_weight=getOutputQuantity(0)/kbt_; //first value is always the current bias
      for(unsigned e=0; e<extra_biases_.size(); e++)
...
template <class mode>
double OPESmetad_test<mode>::getProbAndDerivatives(const std::vector<double>& cv,std::vector<double>& der_prob)
{
  double prob=0.0;
  if(!nlist_)
  {
...
template <class mode>
void OPESmetad_test<mode>::addKernel(const double height,const std::vector<double>& center,const std::vector<double>& sigma)
{
  bool no_match=true;
  if(threshold2_!=0)
  {
...
template <class mode>
void OPESmetad_test<mode>::addKernel(const double height,const std::vector<double>& center,const std::vector<double>& sigma,const double logweight)
{
  addKernel(height,center,sigma);
//write to file
...
template <class mode>
unsigned OPESmetad_test<mode>::getMergeableKernel(const std::vector<double>& giver_center,const unsigned giver_k)
...
template <class mode>
void OPESmetad_test<mode>::updateNlist(const std::vector<double>& new_center)
{
...
template <class mode>
void OPESmetad_test<mode>::dumpStateToFile()
{
...
template <class mode>
inline double OPESmetad_test<mode>::evaluateKernel(const kernel& G,const std::vector<double>& x) const
...
template <class mode>
inline double OPESmetad_test<mode>::evaluateKernel(const kernel& G,const std::vector<double>& x, std::vector<double>& acc_der, std::vector<double>& dist)
{ //NB: cannot be a method of kernel class, because uses external variables (for cutoff)
  double norm2=0;
  for(unsigned i=0; i<ncv_; i++)
  {
...
template <class mode>
inline void OPESmetad_test<mode>::mergeKernels(kernel& k1,const kernel& k2)
{
